...
var giAvatar = new Class(
{
	...
	{
		this.bound = {
			mouseOver:		function(event) { if (this.navigation) $(event.target).tween('opacity', 1.0); }.bind(this),
			mouseOut:		function(event)	{ if (this.navigation) $(event.target).tween('opacity', 0.5); }.bind(this),
			...
			for(x=-1; x<2; x++)
			{
				if (x != 0 || y != 0)
				{
					...
					var arrow = new Element('div', {
						...
						tween:		{ duration: 150 },
						morph:		{ duration: 150 },
						events:		{ mouseover: this.bound.mouseOver, mouseout: this.bound.mouseOut, mouseup: this.bound.arrowClick },
					});
